...
namespace ETModel
{
	...
	/**
	* @description: 
		* Session要持有实际 Service 中的信道,并通过信道send数据
		* ReadCallback调用
		* requestCallback调用 
	* @param {type} 
	* @return: 
	*/
	public sealed class Session : Entity
	{
		...
		/**
		* @description: 
			* OnRead方法通过信道的ReadCallback调用
			* 在Session的Awake方法中OnRead被添加到信道的ReadCallback  //channel.ReadCallback += this.OnRead;
		* @param {type} 
		* @return: 
		*/
...
// #endif
		...
		/**
		* @description: 
			* Call方法首先根据rpcID，在requestCallback[rpcID]注册了response，然后执行到倒数第二行this.Send方法。
			* 之后转到Run(MemoryStream memoryStream)方法中的rpc返回消息（注，flag第一位为1），requestCallback取出前面注册的response,并被委托执行，完成了 tcs.SetResult(response)。
			* 继续Call方法中 return tcs.Task 返回一个Task<IResponse>。
		* @param {type} 
		* @return: 
		*/
