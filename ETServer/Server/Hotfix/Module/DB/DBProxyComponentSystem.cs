...
namespace ETHotfix
{
	...
	public static class DBProxyComponentEx
	{
		...
		public static async Task<List<Component>> Query2<T>(this DBProxyComponent self, string json) where T : Component
		{
			Session session = Game.Scene.GetComponent<NetInnerComponent>().Get(self.dbAddress);
			DBQuery2JsonResponse dbQueryJsonResponse = (DBQuery2JsonResponse)await session.Call(new DBQuery2JsonRequest { CollectionName = typeof(T).Name, Json = json });
			return (List<Component>)dbQueryJsonResponse.Components;
		}
	}
}